...
DZMSHotSpotsFindPos = {
    ...
    if (_mapHardCenter AND (!(DZMSHotSpotsStaticPlc))) then {
        ...
        while {_findRun} do
        {
			...
			//diag_log text format ["[DZMSHotSpots]: DEBUG: Pos:[%1,%2] / noWater?:%3 / okDistance?:%4 / TaviHeight:%5 / BlackListed?:%6", _posX, _posY, _noWater, _okDis, _tavHeight, _isBlack];
            sleep 2;
        };
...
DZMSHotSpotsProtectObj = {
    ...
    if (!((typeOf _object) in ["USVehicleBox","USLaunchersBox","AmmoBoxSmall_556","AmmoBoxSmall_762","MedBox0","USBasicWeaponsBox","USBasicAmmunitionBox","RULaunchersBox"]) || DZMSHotSpotsSceneryDespawnLoot) then {
