...
_hasitem1 = [["ItemBurlap",2], "ItemWaterBottle",["FoodCanCorn"]] call player_checkItems;

//CHECK ITEMS AND NEAREST///

if (!_hasitem1) exitWith {cutText [format["Needs 2xItemBurlap 1xWaterBottle and 1xFoodCanCorn"], "PLAIN DOWN"];};
...
r_interrupt = false;
_removed = [["ItemBurlap",2], "ItemWaterBottle",["FoodCanCorn"]] call player_removeItems;
...
_earlyObj = "Sign_arrow_down_EP1" createVehicle (_position);
...
sleep 6;
cutText [format["Your cow will be ready in 3 minutes!"], "PLAIN DOWN"];

sleep 180;

cutText [format["cow are ready to be eated!!"], "PLAIN DOWN"];

deleteVehicle _earlyObj;

_lastObj = "Cow03" createVehicle (_position);
